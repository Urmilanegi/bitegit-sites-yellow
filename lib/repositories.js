...
function createRepositories(collections) {
  ...
  async function storeEmailOtp(userId, otp, expiresAt) {
    await p2pCredentials.updateOne(
      { userId: String(userId) },
      { $set: { emailOtp: String(otp), emailOtpExpiresAt: Number(expiresAt), updatedAt: new Date() } }
    );
  }

  async function verifyAndConsumeEmailOtp(userId, otp) {
    const cred = await p2pCredentials.findOne({ userId: String(userId) });
    ...
    await p2pCredentials.updateOne(
      { userId: String(userId) },
      { $set: { emailVerified: true, emailVerifiedAt: new Date(), updatedAt: new Date() }, $unset: { emailOtp: '', emailOtpExpiresAt: '' } }
    );
    return { ok: true };
  }
  // ── End Email OTP ────────────────────────────────────────────────
