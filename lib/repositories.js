...
function isTruthyFlag(value) {
  ...
  if (typeof value === 'string') {
    ...
    return ['1', 'true', 'yes', 'verified'].includes(normalized);
  }
  return false;
}

function isCredentialEmailVerified(credential) {
  if (!credential || typeof credential !== 'object') {
    return false;
  }
  return (
    isTruthyFlag(credential.emailVerified) ||
    isTruthyFlag(credential.email_verified) ||
    isTruthyFlag(credential.verified) ||
    Boolean(credential.emailVerifiedAt) ||
    Boolean(credential.email_verified_at) ||
    Boolean(credential.verifiedAt)
  );
}

function buildP2PCredentialUpsert(email, passwordHash, options = {}, now = new Date()) {
  ...
  const nextUsername = String(options.username || '').trim();
  const nextAvatar = String(options.avatar || '').trim();
  ...
  if (hasMerchantFlag) {
    setDoc.isMerchant = Boolean(options.isMerchant);
  }
  if (hasMerchantDepositFlag) {
    setDoc.merchantDepositLocked = Boolean(options.merchantDepositLocked);
  }
  if (nextMerchantLevel) {
    setDoc.merchantLevel = nextMerchantLevel;
  }
  if (nextUsername) {
    setDoc.username = nextUsername;
  }
  if (nextAvatar) {
    setDoc.avatar = nextAvatar;
  }
  if (nextUserId) {
    setDoc.userId = nextUserId;
  }
  if (hasEmailVerifiedFlag) {
    setDoc.emailVerified = Boolean(options.emailVerified);
    setDoc.emailVerifiedAt = Boolean(options.emailVerified) ? now : null;
  }
...
function createRepositories(collections) {
  ...
  async function setP2PCredential(email, passwordHash, options = {}) {
    const now = new Date();
    const operation = buildP2PCredentialUpsert(email, passwordHash, options, now);
    await p2pCredentials.updateOne(operation.filter, operation.update, { upsert: true });
  }
  ...
  async function updatePaymentMethod(id, userId, updates) {
    const allowed = ['nickname','provider','upiId','bankName','accountNumber','ifsc','accountHolder','details','type','qrCode'];
  ...
  // ===== USER REPUTATION =====
  async function getP2PCredentialByUserId(userInput) {
    ...
    let credentialByUserId = null;
    let credentialByEmail = null;

    if (normalizedUserId) {
      credentialByUserId = await p2pCredentials.findOne({ userId: normalizedUserId });
    }
    if (normalizedEmail) {
      credentialByEmail = await p2pCredentials.findOne({ email: normalizedEmail });
    }

    let credential = credentialByEmail || credentialByUserId;
    if (!credential) {
      return null;
    }

    const patch = {};
    const effectiveEmail = String(credential.email || normalizedEmail).trim().toLowerCase();
    const normalizedVerified = isCredentialEmailVerified(credential);

    if (normalizedUserId && String(credential.userId || '').trim() !== normalizedUserId) {
      patch.userId = normalizedUserId;
      credential.userId = normalizedUserId;
    }

    if (normalizedVerified !== Boolean(credential.emailVerified)) {
      patch.emailVerified = normalizedVerified;
      if (normalizedVerified && !credential.emailVerifiedAt) {
        patch.emailVerifiedAt = new Date();
        credential.emailVerifiedAt = patch.emailVerifiedAt;
      }
      credential.emailVerified = normalizedVerified;
    }

    if (effectiveEmail && Object.keys(patch).length > 0) {
  ...
  async function getUserReputation(userId) {
    ...
    const ratingSum = cred.rep_ratingSum || 0;
    const ratingCount = cred.rep_ratingCount || 0;
    const avgStars = ratingCount > 0 ? Math.round((ratingSum / ratingCount) * 10) / 10 : null;
    return {
      userId,
  ...
  async function submitRating(fromUserId, toUserId, orderId, stars, comment) {
    const now = Date.now();
    await p2pRatings.insertOne({
    ...
    await p2pCredentials.updateOne(
      ...
      { $inc: { rep_ratingSum: Number(stars), rep_ratingCount: 1 } }
    );
  }

  async function hasRatedOrder(fromUserId, orderId) {
    const count = await p2pRatings.countDocuments({
      fromUserId: String(fromUserId),
      orderId: String(orderId)
    });
    return count > 0;
  ...
  async function storeEmailOtp(userInput, otp, expiresAt) {
    const cred = await getP2PCredentialByUserId(userInput);
    if (!cred || !cred.email) {
      return false;
    }

    await p2pCredentials.updateOne(
      { email: String(cred.email).trim().toLowerCase() },
      {
        $set: {
          userId: String(cred.userId || userInput?.id || userInput || '').trim(),
          emailOtp: String(otp),
          emailOtpExpiresAt: Number(expiresAt),
          updatedAt: new Date()
        }
      }
    );
  ...
  async function verifyAndConsumeEmailOtp(userInput, otp) {
    const cred = await getP2PCredentialByUserId(userInput);
    if (!cred) return { ok: false, reason: 'user_not_found' };
    if (!cred.emailOtp) return { ok: false, reason: 'no_otp' };
    if (Date.now() > Number(cred.emailOtpExpiresAt || 0)) return { ok: false, reason: 'expired' };
    if (String(cred.emailOtp).trim() !== String(otp).trim()) return { ok: false, reason: 'wrong_otp' };
    await p2pCredentials.updateOne(
      { email: String(cred.email || '').trim().toLowerCase() },
      {
        $set: {
        ...
        $unset: { emailOtp: '', emailOtpExpiresAt: '' }
      }
    );
    return { ok: true };
  }
