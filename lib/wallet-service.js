...
const CANCELLABLE_ORDER_STATUSES = Object.freeze([
  ...CREATED_LIKE_STATUSES,
  ...PAYMENT_SENT_LIKE_STATUSES
]);
const PAYMENT_WARNING_LEAD_MS = 5 * 60 * 1000;
const BUYER_FIVE_MIN_WARNING_TEXT = `The order will be canceled in 5 minutes. If you've already paid the seller, please go to the order details and click "I've paid", or you may risk losing your assets.`;
const SELLER_FIVE_MIN_WARNING_TEXT = `The order will be canceled in 5 minutes if the buyer does not complete payment. If the buyer has already paid, ask them to go to the order details and click "I've paid" as soon as possible.`;
const BUYER_LATE_PAYMENT_CANCELLED_TEXT = `The order has been canceled by the platform due to your late payment. If you've already made the payment, please contact the seller immediately.`;
const SELLER_LATE_PAYMENT_CANCELLED_TEXT = `The order has been canceled by the platform because the buyer did not complete payment in time. Do not release the assets for this order.`;
...
function buildOrderMessage({ id, sender = 'System', role = '', senderRole = '', text = '', createdAt = Date.now(), isSystem = false } = {}) {
  return {
    id: String(id || `msg_${createdAt}_${Math.floor(Math.random() * 1000)}`),
    sender: String(sender || 'System').trim() || 'System',
    role: String(role || '').trim(),
    senderRole: String(senderRole || '').trim(),
...
function buildOrderSystemMessageId(orderId, suffix) {
  return `msg_${String(orderId || '').trim()}_${String(suffix || '').trim()}`;
}
...
function normalizeAppealAttachments(items = [], at = Date.now()) {
  ...
  return items
    .slice(0, 3)
    .map((item, index) => {
      const dataUrl = String(item?.dataUrl || item?.imageBase64 || item?.imageUrl || '').trim();
      if (!dataUrl) {
        return null;
      }
      return {
        id: `appeal_file_${at}_${index}`,
        name: String(item?.name || `appeal-${index + 1}.png`).trim(),
        dataUrl,
        uploadedAt: at
      };
    })
...
function normalizeAppealDetails(payload = {}, actor = {}, order = {}) {
  ...
  return {
    ...
    orderSnapshot: {
      ...
      fiatAmount: toAmount(order?.fiatAmount || order?.amountInr, 2),
      cryptoAmount: toAmount(order?.cryptoAmount || order?.assetAmount),
...
function createWalletService(collections, mongoClient, options = {}) {
  ...
  async function invokeAfterHook(payload) {
    ...
    try {
      ...
      console.warn('[wallet-service] afterOperation hook failed:', error?.message || error);
    }
  }
  ...
  async function createEscrowOrder(orderInput) {
    ...
    try {
      const result = await runInTransaction(async (session) => {
        ...
        const matchedOffer = await p2pOffers.findOne(
          {
            id: adId,
            status: 'ACTIVE',
            $or: [{ moderationStatus: 'APPROVED' }, { moderationStatus: { $exists: false } }, { moderationStatus: null }],
            merchantDepositLocked: true,
            isDemo: { $ne: true },
            environment: { $ne: 'demo' },
        ...
        const currentAvailableRaw = Number(
          matchedOffer.availableAmount !== undefined ? matchedOffer.availableAmount : matchedOffer.available
        );
        const currentAvailable = toAmount(currentAvailableRaw);
        ...
        const adUpdate = await p2pOffers.updateOne(
          {
            ...
            status: 'ACTIVE',
            ...(matchedOffer.availableAmount !== undefined
              ? { availableAmount: currentAvailableRaw }
              : { available: currentAvailableRaw })
          },
          {
            $set: {
        ...
        const orderDoc = {
          ...
          messages: [
            ...
            buildOrderMessage({
              id: `msg_${now}_sys_s`,
              role: 'seller',
              text: `A new order has been created. Please wait for the buyer to complete the payment. The order will be automatically cancelled if payment is not completed in time.`,
              createdAt: now,
              isSystem: true
            })
          ]
  ...
  async function markOrderPaid(orderId, actor, options = {}) {
    ...
    const statusResult = await p2pOrders.updateOne(
      ...
      },
      {
        $set: {
          ...
          updatedAt: now,
          paymentSentAt: now,
          ...(options.proofBase64 ? { paymentProof: options.proofBase64 } : {})
        },
        $push: {
          messages: {
            $each: [
              {
                ...buildOrderMessage({
                  id: `msg_${now}_paid_b`,
                  role: 'buyer',
                  text: `You've marked the order as "Paid". Please wait for the seller to confirm the payment and release the assets.`,
                  createdAt: now,
                  isSystem: true
                })
              },
              {
                ...buildOrderMessage({
                  id: `msg_${now}_paid_s`,
                  role: 'seller',
                  text: `The buyer has marked the order as "Paid". Please verify the payment in your account and release the assets.`,
                  createdAt: now,
                  isSystem: true
                })
              }
  ...
  async function setOrderDisputed(orderId, actor, options = {}) {
    ...
    const now = Date.now();
    const appealDetails = normalizeAppealDetails(disputePayload, actor, order);
    await p2pOrders.updateOne(
      { id: orderId },
      {
        $set: {
          ...
          updatedAt: now,
          disputedAt: now,
          disputedBy: actor.username || actor.id || 'User',
          ...(options.reason ? { disputeReason: String(options.reason).slice(0, 500) } : {})
        },
        $push: {
          messages: {
            ...
            sender: 'System',
            role: 'system',
            text: `Dispute raised by ${actor.username || 'User'}${options.reason ? ': ' + String(options.reason).slice(0, 200) : ''}. An admin will review this order.`,
            createdAt: now
          }
        }
      }
                ...
                $each: [
                  {
                    ...buildOrderMessage({
                      id: `msg_${now}_rel_b`,
                      role: 'buyer',
                      text: `The seller has confirmed the payment and released the assets. ${feeAmount > 0 ? `Fee deducted: ${feeAmount} ${String(order.asset || 'USDT').toUpperCase()}. ` : ''}Your USDT has been credited to your wallet.`,
                      createdAt: now,
                      isSystem: true
                    })
                  },
  ...
  async function cancelOrder(orderId, actor, status = 'CANCELLED') {
    ...
    const cancelReason = String(actor?.reason || '').trim();
    ...
    try {
      const result = await runInTransaction(async (session) => {
        ...
        const now = Date.now();
        const escrowAmount = ensurePositiveAmount(order.escrowAmount || order.assetAmount, 'Escrow amount');
        const actorRole = String(isSystem ? 'system' : (transition.actorRole || '')).trim().toLowerCase();
        ...
        await p2pOrders.updateOne(
          ...
          },
          {
            $set: {
              ...
              cancelledByRole: targetStatus === ORDER_STATUS.CANCELLED ? actorRole : order.cancelledByRole || '',
              cancelReason: targetStatus === ORDER_STATUS.CANCELLED ? cancelReason : order.cancelReason || ''
            },
            $push: {
              messages:
                targetStatus === ORDER_STATUS.EXPIRED
                  ? {
                      $each: [
                        buildOrderMessage({
                          id: buildOrderSystemMessageId(orderId, 'expired_b'),
                          role: 'buyer',
                          text: BUYER_LATE_PAYMENT_CANCELLED_TEXT,
                          createdAt: now,
                          isSystem: true
                        }),
                        buildOrderMessage({
                          id: buildOrderSystemMessageId(orderId, 'expired_s'),
                          role: 'seller',
                          text: SELLER_LATE_PAYMENT_CANCELLED_TEXT,
                          createdAt: now,
                          isSystem: true
                        })
                      ]
                  ...
                  : {
                      $each: [
                        buildOrderMessage({
                          id: buildOrderSystemMessageId(orderId, 'cancel_b'),
                          role: 'buyer',
                          sender: 'System',
                          senderRole: 'system',
                          text: buyerCancelText,
                          createdAt: now,
                          isSystem: true
                        }),
                        buildOrderMessage({
                          id: buildOrderSystemMessageId(orderId, 'cancel_s'),
                          role: 'seller',
                          sender: 'System',
                          senderRole: 'system',
                          text: sellerCancelText,
                          createdAt: now,
                          isSystem: true
                        })
                      ]
  ...
  async function expireOrders(nowMs = Date.now()) {
    ...
    for (const order of candidates) {
      try {
        ...
        expired += 1;
        if (cancelledDoc) {
          expiredOrders.push(cancelledDoc);
        }
      } catch (error) {
  ...
  async function warnOrdersNearingExpiry(nowMs = Date.now(), warningLeadMs = PAYMENT_WARNING_LEAD_MS) {
    const warningUpperBound = Number(nowMs) + Number(warningLeadMs || 0);
    const candidates = await p2pOrders
      .find({
        status: { $in: EXPIRABLE_ORDER_STATUSES },
        expiresAt: { $gt: nowMs, $lte: warningUpperBound },
        $or: [
          { fiveMinuteWarningSentAt: { $exists: false } },
          { fiveMinuteWarningSentAt: null }
        ]
      })
      .project({ id: 1 })
      .limit(100)
    ...
    const warnedOrders = [];
    for (const candidate of candidates) {
      const warnedDoc = await p2pOrders.findOneAndUpdate(
        {
          id: candidate.id,
          status: { $in: EXPIRABLE_ORDER_STATUSES },
          expiresAt: { $gt: nowMs, $lte: warningUpperBound },
        ...
        },
        {
          $set: {
            updatedAt: nowMs,
            fiveMinuteWarningSentAt: nowMs
          },
          $push: {
            messages: {
              $each: [
                buildOrderMessage({
                  id: buildOrderSystemMessageId(candidate.id, 'warn5_b'),
                  role: 'buyer',
                  text: BUYER_FIVE_MIN_WARNING_TEXT,
                  createdAt: nowMs,
                  isSystem: true
                }),
                buildOrderMessage({
                  id: buildOrderSystemMessageId(candidate.id, 'warn5_s'),
                  role: 'seller',
                  text: SELLER_FIVE_MIN_WARNING_TEXT,
                  createdAt: nowMs,
                  isSystem: true
                })
              ]
      ...
      if (warnedDoc) {
        warnedOrders.push(warnedDoc);
      }
    }

    return {
      warned: warnedOrders.length,
      warnedOrders
    };
  }
