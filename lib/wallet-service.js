      ...
      { id: orderId },
      {
        ...
        $push: {
          messages: {
            ...
            sender: 'System',
            role: 'system',
            text: `Dispute raised by ${actor.username || 'User'}${disputeReason ? ': ' + disputeReason.slice(0, 200) : ''}. An admin will review this order.`,
            createdAt: now
          },
          statusHistory: createOrderStatusHistoryEntry({
            status: ORDER_STATUS.DISPUTED,
            ...
            metadata: {
              orderId: String(orderId || '').trim(),
              appealType: String(appealDetails?.type || '').trim(),
              hasAttachments: Array.isArray(appealDetails?.attachments) && appealDetails.attachments.length > 0
            }
          })
        }
      }
