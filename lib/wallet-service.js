...
function createWalletService(collections, mongoClient, options = {}) {
  ...
  async function setOrderDisputed(orderId, actor, disputePayload = {}) {
    const userId = String(actor?.id || actor?.userId || '').trim();
    if (!userId) {
      throw createAppError('INVALID_ACTOR', 'Actor is required.');
    }
    ...
    await p2pOrders.updateOne(
      { id: orderId },
      {
        $set: {
          ...
          updatedAt: now,
          disputedAt: now,
          appealDetails: appealDetails || order.appealDetails || null
        },
        $push: {
          messages: {
            $each: [
              buildOrderMessage({
                id: `msg_${now}_appeal_system`,
                text: 'An appeal has been filed for this order. Please wait for Support to review the dispute details.',
                createdAt: now,
                isSystem: true
              }),
              buildOrderMessage({
                id: `msg_${now}_appeal_actor`,
                sender: actor.username || 'User',
                senderRole: transition.actorRole || '',
                text: appealDetails?.reason
                  ? `Filed an appeal: ${appealDetails.reason}`
                  : `${actor.username || 'User'} filed an appeal.`,
                createdAt: now
              })
            ]
          },
          statusHistory: createOrderStatusHistoryEntry({
            ...
            metadata: {
              ...
              appealAttachmentCount: Array.isArray(appealDetails?.attachments) ? appealDetails.attachments.length : 0
            }
          })
        }
      }
