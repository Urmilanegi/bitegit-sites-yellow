...
<table cellpadding="0" cellspacing="0" border="0" align="center">
  <tr>
    ...
    <td align="center" valign="middle">
      <span style="font-size:22px;font-weight:900;color:${GOLD};letter-spacing:2px;font-family:Arial,sans-serif;">
...
function ctaButton(label, url) {
  return `
  <table cellpadding="0" cellspacing="0" border="0" style="margin:24px 0 16px;">
    <tr>
      <td>
        <a href="${url}" style="display:inline-block;background:${GOLD};color:#000000;
           font-size:15px;font-weight:700;text-decoration:none;padding:13px 28px;
...
// ─── Social SVG icons (no circles, just plain icons) ──────────────────────
const SOCIAL_ICONS = `
<table cellpadding="0" cellspacing="0" border="0" align="center" style="margin:12px 0;">
  <tr>
    <td style="padding:0 10px;">
      <a href="https://twitter.com/${APP_NAME.toLowerCase()}" style="text-decoration:none;">
        <svg width="22" height="22" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.259 5.622L18.244 2.25z" fill="#555555"/>
        </svg>
      </a>
    </td>
    <td style="padding:0 10px;">
    ...
    <td style="padding:0 10px;">
      <a href="https://youtube.com/@${APP_NAME.toLowerCase()}" style="text-decoration:none;">
        <svg width="22" height="22" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path d="M23.495 6.205a3.007 3.007 0 0 0-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 0 0 .527 6.205a31.247 31.247 0 0 0-.522 5.805 31.247 31.247 0 0 0 .522 5.783 3.007 3.007 0 0 0 2.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 0 0 2.088-2.088 31.247 31.247 0 0 0 .5-5.783 31.247 31.247 0 0 0-.5-5.805zM9.609 15.601V8.408l6.264 3.602z" fill="#555555"/>
        </svg>
      </a>
    </td>
    <td style="padding:0 10px;">
...
function buildFooter() {
  ...
  <tr><td style="padding:20px 32px 4px;text-align:center;">
    <p style="margin:0 0 12px;font-size:16px;font-weight:700;color:${GOLD};font-family:Arial,sans-serif;">
      Stay connected!
    </p>
    ${SOCIAL_ICONS}
  </td></tr>
  ...
  <tr><td style="padding:12px 32px 4px;">
    <p style="margin:0;font-size:14px;color:${TEXT};font-family:Arial,sans-serif;line-height:1.7;">
...
const SECURITY_LINE = `
  <p style="margin:16px 0 0;font-size:15px;color:${TEXT};font-family:Arial,sans-serif;line-height:1.7;">
...
const AUTO_MSG = `
  <p style="margin:20px 0 8px;font-size:15px;color:${TEXT};font-style:italic;
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F5F5;padding:20px 0;">
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0"
      ...
      <!-- BLACK HEADER with gold BITEGIT logo -->
      <tr>
        <td style="background:${BLACK};padding:18px 32px;text-align:center;">
          ${LOGO_HTML}
        </td>
      </tr>
...
function withdrawalSuccessful({ toEmail, amount, asset, withdrawalTime, address, txId }) {
  ...
  <tr><td style="padding:32px 32px 8px;">
    ...
    <p style="margin:0 0 20px;font-size:15px;color:${TEXT};line-height:1.7;font-family:Arial,sans-serif;">
      You have successfully withdrawn <strong>${esc(String(amount))} ${assetUpper}</strong> from your account.
    </p>
    <p style="margin:0 0 6px;font-size:15px;color:${TEXT};line-height:1.7;font-family:Arial,sans-serif;">
      <strong>Withdrawal Address:</strong><br/>
      <span style="word-break:break-all;">${esc(address || 'N/A')}</span>
    </p>
    <p style="margin:12px 0 0;font-size:15px;color:${TEXT};line-height:1.7;font-family:Arial,sans-serif;">
      <strong>Transaction ID:</strong> ${esc(txId || 'N/A')}
    </p>
    ${ctaButton('Visit Your Dashboard', 'https://' + APP_DOMAIN + '/wallet')}
    ${SECURITY_LINE}
    <p style="margin:14px 0 0;font-size:15px;color:${TEXT};line-height:1.7;font-family:Arial,sans-serif;">
...
function withdrawalVerificationCode({ toEmail, code, withdrawalTime, amount, asset, address }) {
  ...
  <tr><td style="padding:32px 32px 8px;">
    ...
    <p style="margin:0 0 12px;font-size:15px;color:${TEXT};line-height:1.7;font-family:Arial,sans-serif;">
      Your verification code
    </p>
    <p style="margin:0 0 16px;font-size:36px;font-weight:700;color:${GOLD};
    ...
    ${SECURITY_LINE}
    <p style="margin:12px 0 0;font-size:15px;color:#CC0000;font-weight:700;line-height:1.7;font-family:Arial,sans-serif;">
...
function loginAlert({ toEmail, loginTime, device, ipAddress, location }) {
  const content = `
  <tr><td style="padding:32px 32px 8px;">
    <h1 style="margin:0 0 20px;font-size:24px;font-weight:700;color:${TEXT};
               font-family:Arial,sans-serif;line-height:1.3;">
    ...
    <p style="margin:0 0 20px;font-size:15px;color:${TEXT};line-height:1.7;font-style:italic;font-family:Arial,sans-serif;">
...
// ══════════════════════════════════════════════════════════════════
// 6. P2P — NEW ORDER PLACED (seller notification)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// 7. P2P — BUYER PAID, PLEASE RELEASE (seller)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// 12. P2P — ORDER COMPLETED / CRYPTO RELEASED (buyer)
// ══════════════════════════════════════════════════════════════════
